...
export function PelangganClient({ initialData }: { initialData: Pelanggan[] }) {
  ...
  // states for modals and notification
  ...
  // states for transaction history detail
  ...
  const executeDelete = () => {
    ...
    startTransition(async () => {
      ...
      else showNotification('Data pelanggan berhasil dihapus.', 'success');
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (res?.error) showNotification(res.error, 'error');
        ...
        showNotification('Data pelanggan berhasil didaftarkan!', 'success');
        form.reset();
      }
    });
  };
  ...
  // detail logic handler
  const openDetail = async (p: Pelanggan) => {
    setModal({ type: 'detail', data: p });
    setMonthOffset(0);
    fetchHistory(p.nik, 0);
  };
  ...
  const changeHistoryMonth = (offset: number) => {
    ...
    if (modal.data) fetchHistory(modal.data.nik, offset);
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 relative">
      ...
      <div className="flex-1 space-y-4">
        ...
        {/* advanced filters - redesigned to match transaksi toolbar */}
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 overflow-hidden shadow-sm">
          <table className="w-full text-left text-sm text-zinc-600 dark:text-zinc-400">
            <thead className="bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 uppercase font-semibold border-b border-zinc-200 dark:border-zinc-800 select-none">
              <tr>
                <th className="px-6 py-4 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('nik')}>
                  <div className="flex items-center gap-2">NIK <ArrowUpDown size={14} className="text-zinc-400" /></div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('nama')}>
                  <div className="flex items-center gap-2">Nama Lengkap <ArrowUpDown size={14} className="text-zinc-400" /></div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('kategori')}>
                  <div className="flex items-center gap-2">Kategori <ArrowUpDown size={14} className="text-zinc-400" /></div>
                </th>
                ...
                processedData.map((p) => (
                  <tr key={p.nik} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-[10px] uppercase tracking-wider rounded-md font-semibold ${getCategoryColor(p.kategori)}`}>
                        {p.kategori}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={() => openDetail(p)} className="text-zinc-400 hover:text-[#52796F]" title="Riwayat Pembelian"><Eye size={18} /></button>
                        <button onClick={() => setModal({ type: 'edit', data: p })} className="text-zinc-400 hover:text-amber-500" title="Ubah Data"><Edit size={18} /></button>
                        <button onClick={() => setConfirmDelete(p)} className="text-zinc-400 hover:text-red-500" title="Hapus Data"><Trash2 size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm sticky top-8">
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            <div>
              ...
              <input type="text" name="nik" maxLength={16} required className="w-full px-3 py-2 text-sm rounded-md border border-zinc-300 dark:border-zinc-700 bg-transparent outline-none focus:border-[#52796F] transition-colors" />
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1">Nama Lengkap</label>
              <input type="text" name="nama" required className="w-full px-3 py-2 text-sm rounded-md border border-zinc-300 dark:border-zinc-700 bg-transparent outline-none focus:border-[#52796F] transition-colors" />
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1">Kategori</label>
              <div className="relative">
                <select name="kategori" className="appearance-none w-full px-3 py-2 text-sm rounded-md border border-zinc-300 dark:border-zinc-700 bg-transparent outline-none focus:border-[#52796F] transition-colors cursor-pointer">
                  <option value="RT">Rumah Tangga (RT)</option>
                  <option value="UM">Usaha Mikro (UM)</option>
                  <option value="PENGECER">Pengecer</option>
                </select>
            ...
            <button type="submit" disabled={isPending} className="w-full py-2 text-sm rounded-md text-white font-medium bg-[#52796F] hover:bg-[#43645a] disabled:opacity-50 transition-colors">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-xl max-w-sm w-full p-6 text-center shadow-xl border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-zinc-500 mb-6">Anda akan menghapus data pelanggan <strong>{confirmDelete.nama}</strong>. Data yang dihapus tidak dapat dikembalikan.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} disabled={isPending} className="flex-1 py-2 rounded-md border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">Batal</button>
              <button onClick={executeDelete} disabled={isPending} className="flex-1 py-2 rounded-md bg-red-600 hover:bg-red-700 text-white font-medium transition-colors">{isPending ? 'Memproses...' : 'Ya, Hapus'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`bg-white dark:bg-zinc-950 rounded-xl shadow-xl w-full overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200 ${modal.type === 'detail' ? 'max-w-2xl' : 'max-w-sm'}`}>
            <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
              <div>
                <h3 className="font-bold text-zinc-900 dark:text-white">{modal.type === 'edit' ? 'Ubah Data Pelanggan' : 'Riwayat Pelanggan'}</h3>
                {modal.type === 'detail' && <p className="text-xs text-zinc-500 mt-1">{modal.data.nama} - {modal.data.nik}</p>}
              </div>
              <button onClick={() => setModal({ type: null, data: null })} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors"><X size={20} /></button>
            </div>
            ...
            <div className="p-5">
              {modal.type === 'detail' ? (
                  ...
                  {historyLoading ? (
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-6">
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20">
                          <p className="text-xs text-[#52796F] font-semibold mb-1">Hari Ini</p>
                          <p className="text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalHariIni} <span className="text-sm font-normal text-zinc-500">Tbg</span></p>
                        </div>
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20">
                          <p className="text-xs text-[#52796F] font-semibold mb-1">Minggu Ini</p>
                          <p className="text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalMingguIni} <span className="text-sm font-normal text-zinc-500">Tbg</span></p>
                        </div>
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20">
                          <p className="text-xs text-[#52796F] font-semibold mb-1">Bulan Ini</p>
                          <p className="text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalBulanIni} <span className="text-sm font-normal text-zinc-500">Tbg</span></p>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="relative">
                          <select value={monthOffset} onChange={(e) => changeHistoryMonth(Number(e.target.value))} className="appearance-none text-xs border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 rounded-md pl-2 pr-7 py-1.5 outline-none text-zinc-700 dark:text-zinc-300 cursor-pointer">
                        ...
                        <table className="w-full text-left text-xs">
                          ...
                          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                            {historyData.listRiwayat.length === 0 ? (
                              <tr><td colSpan={3} className="px-4 py-6 text-center text-zinc-500">Tidak ada transaksi di bulan ini.</td></tr>
                            ) : (
                              historyData.listRiwayat.map((trx: any) => (
                                <tr key={trx.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
                                  <td className="px-4 py-2 text-zinc-700 dark:text-zinc-300">{new Date(trx.tanggal).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</td>
                                  <td className="px-4 py-2 font-semibold text-[#52796F]">{trx.jumlah} Tbg</td>
                                  <td className="px-4 py-2 text-zinc-500">{trx.kasir}</td>
                                </tr>
                              ))
                            )}
                          </tbody>
                ...
                <form onSubmit={handleEdit} className="space-y-4">
                  ...
                  <div><label className="block text-xs font-medium text-zinc-500 mb-1">Nama Lengkap</label><input type="text" name="nama" defaultValue={modal.data.nama} required className="w-full px-3 py-2 text-sm rounded-md border border-zinc-300 dark:border-zinc-700 bg-transparent outline-none focus:border-[#52796F] transition-colors" /></div>
                  <div>
                    <label className="block text-xs font-medium text-zinc-500 mb-1">Kategori Pelanggan</label>
                    <div className="relative">
                      <select name="kategori" defaultValue={modal.data.kategori} className="appearance-none w-full px-3 py-2 text-sm rounded-md border border-zinc-300 dark:border-zinc-700 bg-transparent outline-none focus:border-[#52796F] cursor-pointer transition-colors">
                        <option value="RT">Rumah Tangga (RT)</option>
                        <option value="UM">Usaha Mikro (UM)</option>
                        <option value="PENGECER">Pengecer</option>
                      </select>
                  ...
                  <button type="submit" disabled={isPending} className="w-full mt-2 py-2 text-sm rounded-md text-white font-medium bg-[#52796F] hover:bg-[#43645a] disabled:opacity-50 transition-colors">{isPending ? 'Menyimpan...' : 'Simpan Perubahan'}</button>
                </form>
              )}
            </div>
          </div>
