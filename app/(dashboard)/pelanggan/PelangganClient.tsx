...
// define types based on prisma schema relations
type Kategori = { idKategori: string; namaKategori: string; };
type Pelanggan = { nik: string; nama: string; idKategori: string; kategori: Kategori; };

export default function PelangganClient({ initialData, categories }: { initialData: Pelanggan[], categories: Kategori[] }) {
  const [isPending, startTransition] = useTransition();
  const [search, setSearch] = useState('');
  const [filterKategori, setFilterKategori] = useState('');
  const [sortConfig, setSortConfig] = useState<{ key: keyof Pelanggan | 'kategori'; direction: 'asc' | 'desc' } | null>(null);
  ...
  const handleSort = (key: keyof Pelanggan | 'kategori') => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === 'kategori') {
          valA = a.kategori.namaKategori.toLowerCase();
          valB = b.kategori.namaKategori.toLowerCase();
        } else {
          valA = String(a[sortConfig.key as keyof Pelanggan]).toLowerCase();
          valB = String(b[sortConfig.key as keyof Pelanggan]).toLowerCase();
        }
  ...
  const getCategoryColor = (namaKategori: string) => {
    const lower = namaKategori.toLowerCase();
    if (lower.includes('rumah tangga') || lower === 'rt') return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';
    if (lower.includes('mikro') || lower === 'um') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400';
    if (lower.includes('pengecer')) return 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400';
      ...
      <div className="flex-1 space-y-4">
        <div className="bg-white dark:bg-zinc-950 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            {/* added shrink-0 to prevent flexbox from squishing the dropdown */}
            <div className="relative w-full md:w-[200px] shrink-0">
              <select value={filterKategori} onChange={(e) => setFilterKategori(e.target.value)} className="appearance-none w-full py-2 pl-3 pr-8 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm font-medium outline-none text-zinc-700 dark:text-zinc-300 focus:border-[#52796F] cursor-pointer hover:border-zinc-300 transition-colors">
                ...
                {categories.map((cat) => (
                  <option key={cat.idKategori} value={cat.idKategori}>{cat.namaKategori}</option>
                ))}
              </select>
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 overflow-hidden shadow-sm">
          <div className="overflow-x-auto custom-scrollbar">
            {/* added table-fixed class to prevent columns from resizing based on content */}
            <table className="w-full text-left text-sm text-zinc-600 dark:text-zinc-400 min-w-[600px] table-fixed">
              <thead className="bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 uppercase font-semibold border-b border-zinc-200 dark:border-zinc-800 select-none">
                <tr>
                  {/* added fixed widths to specific columns (w-[180px], w-[160px], etc.) */}
                  <th className="w-[180px] px-6 py-4 whitespace-nowrap cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('nik')}><div className="flex items-center gap-2">NIK <ArrowUpDown size={14} className="text-zinc-400" /></div></th>
                  ...
                  <th className="w-[180px] px-6 py-4 whitespace-nowrap cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('kategori')}><div className="flex items-center gap-2">Kategori <ArrowUpDown size={14} className="text-zinc-400" /></div></th>
                ...
                {processedData.length === 0 ? (
                  ...
                  processedData.map((p) => (
                    <tr key={p.nik} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap font-medium text-zinc-900 dark:text-zinc-100 truncate">{p.nama}</td>
                      <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 py-1 text-[10px] uppercase tracking-wider rounded-md font-semibold ${getCategoryColor(p.kategori.namaKategori)}`}>{p.kategori.namaKategori}</span></td>
