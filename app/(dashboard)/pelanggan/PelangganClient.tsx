...
export default function PelangganClient({ initialData }: { initialData: Pelanggan[] }) {
  ...
  const getCategoryColor = (kategori: string) => {
    switch(kategori) {
      case 'Rumah Tangga': return 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/50';
      case 'UM': return 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800/50';
      case 'Pengecer': return 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800/50';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start relative pt-14 md:pt-0">
      
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2 lg:col-span-2 border-b border-zinc-200 dark:border-zinc-800 pb-3">Data Pelanggan</h1>

      {notification && (
        <div className={`fixed top-16 md:top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg border text-sm font-medium w-[90%] md:w-auto transition-all ${notification.type === 'success' ? 'bg-green-50 border-green-200 text-green-800 dark:bg-green-950 dark:border-green-900 dark:text-green-300' : 'bg-red-50 border-red-200 text-red-800 dark:bg-red-950 dark:border-red-900 dark:text-red-300'}`}>
      ...
      <div className="w-full space-y-4">
        <div className="bg-white dark:bg-zinc-950 p-3 md:p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col md:flex-row items-center gap-3">
          <div className="flex-1 relative w-full">
            ...
            <input type="text" placeholder="Ketik nama atau NIK..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 rounded-lg border border-zinc-300 bg-zinc-50 dark:bg-zinc-900/50 dark:border-zinc-700 text-sm outline-none focus:border-[#52796F] transition-colors" />
          </div>
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            <div className="relative w-full md:w-auto">
              <select value={filterKategori} onChange={(e) => setFilterKategori(e.target.value)} className="appearance-none w-full py-2 pl-3 pr-8 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm font-medium outline-none text-zinc-700 dark:text-zinc-300 focus:border-[#52796F] cursor-pointer hover:border-zinc-400 transition-colors">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left text-sm text-zinc-700 dark:text-zinc-300 min-w-[500px]">
              <thead className="bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 font-semibold border-b border-zinc-200 dark:border-zinc-800 select-none">
                <tr>
                  <th className="px-5 py-3.5 whitespace-nowrap cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('nik')}><div className="flex items-center gap-2">NIK <ArrowUpDown size={14} className="text-zinc-400" /></div></th>
                  ...
                  <th className="px-5 py-3.5 whitespace-nowrap cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800/50" onClick={() => handleSort('kategori')}><div className="flex items-center gap-2">Kategori <ArrowUpDown size={14} className="text-zinc-400" /></div></th>
                  <th className="px-5 py-3.5 whitespace-nowrap text-center">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                {processedData.length === 0 ? (
                  <tr><td colSpan={4} className="px-5 py-8 text-center text-zinc-500">Data pelanggan tidak ditemukan.</td></tr>
                ) : (
                  processedData.map((p) => (
                    <tr key={p.nik} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                      <td className="px-5 py-3 whitespace-nowrap font-mono text-xs">{p.nik}</td>
                      <td className="px-5 py-3 whitespace-nowrap font-medium text-zinc-900 dark:text-zinc-100">{p.nama}</td>
                      <td className="px-5 py-3 whitespace-nowrap"><span className={`px-2 py-1 text-[11px] uppercase tracking-wide rounded border font-semibold ${getCategoryColor(p.kategori)}`}>{p.kategori}</span></td>
                      <td className="px-5 py-3 whitespace-nowrap">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => openDetail(p)} className="p-1 text-zinc-400 hover:text-[#52796F] transition-colors" title="Lihat Detail"><Eye size={18} /></button>
                          <button onClick={() => setModal({ type: 'edit', data: p })} className="p-1 text-zinc-400 hover:text-amber-500 transition-colors" title="Ubah Data"><Edit size={18} /></button>
                          <button onClick={() => setConfirmDelete(p)} className="p-1 text-zinc-400 hover:text-red-500 transition-colors" title="Hapus"><Trash2 size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))
      ...
      <div className="w-full lg:sticky lg:top-6 self-start">
        <div className="p-5 md:p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm">
          <div className="flex items-center gap-2 mb-5 text-zinc-900 dark:text-white border-b border-zinc-100 dark:border-zinc-800 pb-3">
            ...
            <h3 className="text-base font-semibold">Registrasi Pelanggan</h3>
          </div>
          <form onSubmit={handleAdd} className="space-y-4">
            <div><label className="block text-xs font-medium text-zinc-500 mb-1">NIK (16 Digit)</label><input type="text" name="nik" maxLength={16} required onInvalid={handleInvalid} onInput={handleInput} className="w-full px-3 py-2 text-sm rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 outline-none focus:border-[#52796F] focus:bg-white transition-colors" /></div>
            ...
            <div>
              ...
              <div className="relative">
                <select name="idKategori" required onInvalid={handleInvalid} onInput={handleInput} className="appearance-none w-full px-3 py-2 text-sm rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 outline-none focus:border-[#52796F] focus:bg-white cursor-pointer transition-colors">
                  <option value="K01">Rumah Tangga (RT)</option>
                  <option value="K02">Usaha Mikro (UM)</option>
                  <option value="K03">Pengecer</option>
                </select>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-xl max-w-sm w-full p-6 text-center shadow-xl border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-zinc-500 mb-6 leading-relaxed">Anda akan menghapus data <strong>{confirmDelete.nama}</strong>. Data tidak dapat dikembalikan.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} disabled={isPending} className="flex-1 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 font-medium">Batal</button>
      ...
      {modal.type && modal.data && (
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className={`bg-white dark:bg-zinc-950 rounded-xl shadow-xl w-full overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200 my-8 ${modal.type === 'detail' ? 'max-w-2xl' : 'max-w-sm'}`}>
            <div className="p-4 md:p-5 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/50 sticky top-0 z-10">
              <div><h3 className="font-bold text-zinc-900 dark:text-white">{modal.type === 'edit' ? 'Ubah Data Pelanggan' : 'Riwayat Pembelian'}</h3>{modal.type === 'detail' && <p className="text-xs text-zinc-500 mt-1 font-mono">{modal.data.nama} ({modal.data.nik})</p>}</div>
            ...
            <div className="p-4 md:p-5">
              {modal.type === 'detail' ? (
                  ...
                  {historyLoading ? <div className="flex flex-col items-center justify-center py-12 text-zinc-400"><Loader2 className="animate-spin mb-2" size={32} /><p className="text-sm">Memuat data...</p></div> : historyData ? (
                    <div>
                      <div className="grid grid-cols-3 gap-3 mb-6">
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20 text-center"><p className="text-[10px] md:text-xs text-[#52796F] font-semibold mb-1">Hari Ini</p><p className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalHariIni} <span className="text-xs font-normal text-zinc-500">Tbg</span></p></div>
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20 text-center"><p className="text-[10px] md:text-xs text-[#52796F] font-semibold mb-1">Minggu Ini</p><p className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalMingguIni} <span className="text-xs font-normal text-zinc-500">Tbg</span></p></div>
                        <div className="bg-[#52796F]/10 p-4 rounded-lg border border-[#52796F]/20 text-center"><p className="text-[10px] md:text-xs text-[#52796F] font-semibold mb-1">Bulan Ini</p><p className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-white">{historyData.totalBulanIni} <span className="text-xs font-normal text-zinc-500">Tbg</span></p></div>
                      </div>
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="text-sm font-semibold text-zinc-900 dark:text-white">Daftar Transaksi</h4>
                        <div className="relative">
                          <select value={monthOffset} onChange={(e) => changeHistoryMonth(Number(e.target.value))} className="appearance-none text-xs border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-md pl-2 pr-7 py-1.5 outline-none text-zinc-700 dark:text-zinc-300 cursor-pointer">
                            <option value={0}>Bulan Ini</option><option value={1}>1 Bulan Lalu</option><option value={2}>2 Bulan Lalu</option>
                          </select>
                ...
                <form onSubmit={handleEdit} className="space-y-4">
                  ...
                  <div><label className="block text-xs font-medium text-zinc-500 mb-1">NIK (Paten)</label><input type="text" disabled value={modal.data.nik} className="w-full px-3 py-2 text-sm rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800/50 text-zinc-500 cursor-not-allowed outline-none" /></div>
